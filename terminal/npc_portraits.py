...
# Fixed slight X-tilt gives overhead depth without looking "good".

def _proj3(x: float, y: float, z: float,
           cx: int, cy: int, s: float, ry: float) -> tuple[int, int]:
    """Y-axis rotation + mild fixed X tilt, orthographic project."""
...
def _wire_box(surface, cx, cy, s, t, hw, hh, hd, color, lw: int = 1):
    """Rotating wireframe box. No backface cull = 'shitty 3D' look."""
    ry = t * 0.32 + 0.5
...
def _wire_hex_prism(surface, cx, cy, s, t, r, hh, color, lw: int = 1):
    """Rotating hexagonal prism wireframe."""
    ry = t * 0.32 + 0.5
    rng = random.Random(int(t * 3))
...
def _gary(surface, cx, cy, s, disposition, t):
    ...
    # Shitty 3D wireframe (drawn first — behind everything)
...
def _synthetic_droid(surface, cx, cy, s, disposition, t):
    ...
    # Shitty 3D wireframe hexagonal prism
...
def _union_dispatcher(surface, cx, cy, s, disposition, t):
    ...
    # Shitty 3D wireframe box (slightly wider — bureaucratic)
    ...
    for gx in (-22, 22):
        pygame.draw.circle(surface, dim,
            (int(cx + gx * s), int(cy - 18 * s)), int(18 * s), 2)
