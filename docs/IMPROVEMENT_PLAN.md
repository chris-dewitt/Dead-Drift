...
**May 2026:** Fixed chapter theme lists in `procedural.py` — **drift from this section** (Ch.1 s5, Ch.2 s5, Ch.3 s2/s3/s5). **Decision pending** — see [DECISION_BRIEFS.md](DECISION_BRIEFS.md) §3. **Do not change code until Chris chooses.**
...
**May 2026:** Beats 1–3 exist in `delivery_sequence.py`. Beat 1 alignment — yes. Beat 2 is **SPACE retro physics only** (no J gauge). **Decision pending** — [DECISION_BRIEFS.md](DECISION_BRIEFS.md) §4. **Chris priority:** improve docking graphics (Phase 0).
