...
def build_checkpoint(game) -> dict:
    ...
    if state_name in {"TERMINAL", "DELIVERY", "INTERSTITIAL"}:
        state_name = "FLIGHT"
...
def restore_checkpoint(game, data: dict) -> bool:
    ...
    rm._ship = ship
    saved_chapter = int(data.get("chapter", 0) or 0)
    if 1 <= saved_chapter <= 6:
        rm._chapter_override = saved_chapter
